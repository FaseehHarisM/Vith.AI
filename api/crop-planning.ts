...
const MAX_STR = 200;
const LANGUAGE_NAMES: Record<string, string> = { en: "English", ml: "Malayalam (മലയാളം)", hi: "Hindi (हिन्दी)", kn: "Kannada (ಕನ್ನಡ)", te: "Telugu (తెలుగు)", ta: "Tamil (தமிழ்)" };
...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are an expert agricultural planner specializing in Kerala and tropical Indian farming.
Based on the field data below, create an optimal crop planning layout that splits the field into zones for maximum yield and sustainability.
  ...
  "zones": [
    {
      ...
      "color": "#22C55E",
      "area_pct": 45,
      "reason": "Best suited for the laterite soil and high rainfall",
      "spacing_m": 4.5,
      "water_needs": "medium",
      "season": "Perennial",
...
- Include at least one NATIVE tree species appropriate for "${location}" (e.g. Coconut, Rubber, Arecanut).
