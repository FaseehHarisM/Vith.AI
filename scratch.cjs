...
const newPrompt = `        : \`You are a precision agriculture AI assistant for farmers in Kerala/India. Give a data-driven analysis for this field. Use simple, actionable language.
...
const startStr = '        : `You are a concise precision agriculture expert.';
const endStr = '      }),\n    });';
...
if (startIndex === -1 || endIndex < startIndex) {
  console.log('Could not find the prompt string');
  process.exit(1);
}
...
code = code.substring(0, startIndex) + newPrompt + replacementFetch + code.substring(endIndex);
