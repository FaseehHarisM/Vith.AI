...
function detectRegion(locationText: string): RegionTag[] {
  ...
  if (matched.size === 0) matched.add("tropical"); // Default to tropical/kerala for hackathon
...
const CROP_PROFILES: CropProfile[] = [
  // === TROPICAL TREES ===
  {
    ...
    waterNeeds: "high",
    tempRange: [20, 35],
    rainfallRange: [2000, 4000],
    phRange: [4.5, 6.0],
    humidityMin: 70,
    baseYield: 1.5, // tonnes/ha
    season: "Perennial (Year-round)",
    tags: ["tree", "tropical", "kerala", "humid"],
    dotSize: 16,
  },
  {
    name: "Arecanut",
    ...
    waterNeeds: "high",
    tempRange: [14, 36],
    rainfallRange: [1500, 5000],
    phRange: [5.0, 7.5],
    humidityMin: 60,
    baseYield: 2.5,
    season: "Perennial (Year-round)",
    tags: ["tree", "tropical", "kerala", "coastal"],
    dotSize: 14,
  },
  {
    name: "Nutmeg",
    ...
    baseYield: 1.2,
    season: "Perennial (Year-round)",
    tags: ["tree", "spice", "tropical", "kerala", "shade-friendly"],
    dotSize: 15,
  },
  {
    name: "Coconut",
    ...
    baseYield: 6.4,
    season: "Perennial (Year-round)",
    tags: ["tree", "tropical", "humid", "coastal", "kerala"],
    dotSize: 16,
  },
  {
    name: "Mango",
  ...
  },
  {
    ...
    waterNeeds: "high",
    tempRange: [10, 35],
    rainfallRange: [1500, 4000],
    phRange: [5.5, 6.5],
    humidityMin: 75,
    baseYield: 0.3,
    season: "Perennial (Aug-Feb harvest)",
    tags: ["spice", "tropical", "kerala", "shade-loving", "highland"],
    dotSize: 8,
  },
  {
    name: "Cocoa",
    ...
    humidityMin: 70,
    baseYield: 1.0,
    season: "Perennial (Sep-Jan harvest)",
    tags: ["tree", "tropical", "kerala", "intercrop"],
    dotSize: 12,
  },
  {
    name: "Banana",
    ...
    baseYield: 24,
    season: "Perennial (Year-round)",
    tags: ["fruit", "tropical", "humid", "kerala"],
    dotSize: 14,
  },
  {
    name: "Turmeric",
  ...
  // === GRAINS (global) ===
  {
    ...
    humidityMin: 60,
    baseYield: 4.8,
    season: "Virippu / Mundakan (Jun-Jan)",
    tags: ["grain", "tropical", "subtropical", "humid", "kerala"],
    dotSize: 7,
  },
  {
    name: "Wheat",
...
function buildLocalCropPlan({ field, ndviData, soilData, weatherData, suitabilityData }: Omit<CropPlanningSectionProps, "mapToken">): CropPlan {
  ...
  // Pseudo-random noise based on field ID so identical regions don't look exactly the same
  const noise = (str: string) => {
    let hash = 0;
    for (let i = 0; i < str.length; i++) hash = Math.imul(31, hash) + str.charCodeAt(i) | 0;
    return (Math.abs(hash) % 100) / 100; // 0.0 to 0.99
  };
  
  const fieldNoise = noise(field.id);
  
  const weights = normalizeAreaPercents(chosen.map((item, idx) => item.score + (noise(field.id + idx) * 0.15)));
    ...
    <div className="animate-fade-in space-y-5" style={{ animationDelay: "450ms" }}>
      ...
      {plan && (
        <>
          ...
          {/* NEW: Explainable AI & ROI Economics */}
          {selectedZone && (
            <div className="rounded-xl border border-primary/30 bg-primary/5 animate-fade-in overflow-hidden">
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  <h5 className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-1">
                    <Lightbulb className="w-3 h-3" /> Why {selectedZone.crop}?
                  </h5>
