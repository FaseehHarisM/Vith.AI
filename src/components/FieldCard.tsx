...
const FieldCard = ({ field, onRemove, variant = "select", isActive = false, style }: FieldCardProps) => {
  ...
  return (
    <div
      ...
      style={{
        ...style,
        backgroundColor: isActive ? "hsl(150, 15%, 20%)" : isHovered ? "hsl(150, 15%, 18%)" : "hsl(150, 15%, 14%)",
        transform: isHovered ? "translateX(-2px) scale(1.01)" : "translateX(0) scale(1)",
        boxShadow: isActive
          ? `0 0 0 1px ${field.color}88, 0 4px 20px -4px ${field.color}33`
          : isHovered ? `0 4px 20px -4px ${field.color}33, inset 0 0 0 1px ${field.color}44` : "none",
        borderColor: isActive ? field.color + "88" : isHovered ? field.color + "66" : "hsl(150, 12%, 22%)",
      }}
    ...
    >
      <div
        ...
        style={{ backgroundColor: field.color + "15", transform: isHovered ? "rotate(-5deg) scale(1.1)" : "rotate(0) scale(1)" }}
      >
        <svg viewBox="0 0 40 40" className="w-8 h-8">
          <polygon points={svgPoints} fill={field.color + "44"} stroke={field.color} strokeWidth="2" />
        </svg>
      </div>
