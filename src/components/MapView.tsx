...
const MapView = ({ allFields, selectedFields, activeField, flyToField, onFlyToDone, onFieldClickOnMap, onAddField, editBoundaryFieldId, onUpdateField, onCancelEditBoundary }: MapViewProps) => {
  ...
  // ── FPO Officer Mode risk classification ──────────────────────
  const [fpoMode, setFpoMode] = useState(false);
  const [fpoSelectedFarm, setFpoSelectedFarm] = useState<Field | null>(null);

  function getFpoRisk(field: Field): { level: "low" | "medium" | "high"; color: string; emoji: string; reason: string } {
    // Simple heuristic based on field properties
    const name = field.name.toLowerCase();
    const crop = field.crop.toLowerCase();
    if (field.ndviChange != null && field.ndviChange < -0.05) return { level: "high", color: "#ef4444", emoji: "🔴", reason: "NDVI declining — possible crop stress or pest damage" };
    if (crop === "paddy" || crop === "rice") return { level: "medium", color: "#f59e0b", emoji: "🟡", reason: "Paddy requires close irrigation monitoring this season" };
    if (name.includes("new") || !field.location) return { level: "medium", color: "#f59e0b", emoji: "🟡", reason: "New or unverified farm — schedule inspection" };
    return { level: "low", color: "#22c55e", emoji: "🟢", reason: "Farm appears healthy. No immediate action needed." };
  }
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <MapToolbar onZoomIn={() => mapRef.current?.zoomIn()} onZoomOut={() => mapRef.current?.zoomOut()} onStyleChange={handleStyleChange}
        ...
        onToggleFpoMode={() => { setFpoMode(prev => !prev); setFpoSelectedFarm(null); }} fpoMode={fpoMode}
        />

      {/* ── FPO Officer Mode Overlay ── */}
      {fpoMode && (
